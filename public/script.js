...
const BEE_RESOURCES = {
    pollinators: {
        ...
        links: [{ name: "Wikipedia: Stingless Bee", url: "https://en.wikipedia.org/wiki/Stingless_bee", icon: "fas fa-book" }, { name: "Stingless Beekeeping - Filipino Style", url: "https://www.ps.org.au/articles/stingless-beekeeping-filipino-style#:~:text=The%20most%20popular%20and%20commonly,pollinate%20mango%20and%20coconut%20crops", icon: "fas fa-book" }, { name: "Tetragonula biroi - Philippines", url: "https://www.nativebeehives.com/tetragonula-biroi-philippines/", icon: "fas fa-book" }],
    },
    honey: {
        title: "Medicinal Pot Honey",
        links: [{ name: "NCBI Study", url: "https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8067784/", icon: "fas fa-microscope" }, { name: "Ark of taste", url: "https://www.fondazioneslowfood.com/en/ark-of-taste-slow-food/honey-of-stingless-bee/", icon: "fas fa-microscope" }, { name: "Philippine Journal of Science", url: "https://philjournalsci.dost.gov.ph/effect-of-storage-time-on-the-quality-of-stingless-bee-tetragonula-biroi-friese-honey/", icon: "fas fa-microscope" }],
    },
    defense: {
        title: "Propolis & Defense",
        links: [{ name: "Wikipedia: Propolis", url: "https://en.wikipedia.org/wiki/Propolis", icon: "fas fa-shield-alt" }, { name: "Philippine Journal of Science", url: "https://philjournalsci.dost.gov.ph/wp-content/uploads/2025/07/inhibitory_activity_of_propolis_from_Phil_stingless_bees_.pdf", icon: "fas fa-shield-alt" }, { name: "ResearchGate", url: "https://www.researchgate.net/publication/376721862_ADAPTIVE_DEFENCE_STRATEGIES_OF_THE_STINGLESS_BEE_TETRAGONULA_IRIDIPENNIS_SMITH_AGAINST_NEST_INTRUDERS_IN_A_NEWLY_DIVIDED_COLONY#:~:text=division%20guard%20bee%20ac%EE%9E%9F,These", icon: "fas fa-shield-alt" }],
    },
};
...
function normalizeCameraClass(value) {
    ...
    if (raw === "vertebrate" || raw === "animal" || raw === "lizard" || raw === "frog" || raw === "bird" || raw === "gecko" || raw === "rodent") return "vertebrate";
    if (raw === "other_insect" || raw === "other insect" || raw === "insect" || raw === "ant" || raw === "wasp" || raw === "fly" || raw === "beetle" || raw === "moth") return "other_insect";
...
function updateVideoModalStatus() {
    ...
    detectionList.innerHTML = latestCameraDetections
        .map((det) => {
            ...
            const mode = (cls === "person" || cls === "other_insect") ? "Alert + Log" :
                (cls === "tb_cluster" || cls === "t_biroi" || cls === "vertebrate") ? "Log Only" : "Monitor";
            return `
...
async function updateData() {
    ...
    if (!isOffline && Math.random() > 0.8) {
        ...
        const statusClass =
            logStatus === "Normal" ? "badge-normal" :
                logStatus === "Logged" ? "badge-logonly" :
                    "badge-warning";
...
function populateAudioLogs() {
    ...
    if (!audioLogs || audioLogs.length === 0) {
        container.innerHTML =
            '<div style="text-align: center; color: var(--text-muted); padding: 20px;">No audio events recorded</div>';
        return;
    }
